...
# - list는 동적으로 크기 변경이 가능한 mutable자료형이다.
# - mutable: list, set, dict (값 변경 가능)
# - immutable: int. float, bool, str, tuple (값 변경 불가능)
...
lst.append(999)
print('append후 list: ', lst)
print('append 후 list id: ', id(lst)) #기존거에 칸을 늘려서 새로운 값을 추가함. 새로 만드는 것보다는 시간이 좀 걸림

print('append 전후 같은 list인가?', before_id == id(lst))

# list.insert(index, 값)
# - index에 값을 삽입하는 메서드
# - 지정된 index부터 뒤에 있는 모든 list 값의 index가 1씩 증가(밀려남)
print("---lst.insert()---")
lst.insert(1, 1.5)
lst.insert(0, 0)
...
# list update(수정)
# list[인덱스] = 값 (변수에 값을 대입해서 변경)
print("---list update---")
lst[0] = -10
print("lst: ", lst)

# 특정 인덱스 값 제거
# list.pop(index): 해당 인덱스 값이 제거됨
...
# list 순회(순차접근, 순차 반복)
# - iterable(=반복할 수 있는) 특징을 가지는 자료형만 가능
print("---list순회---")
lst = ['a', 'b', 'c']
...
# list 인덱스, 요소 순회
for index, v in enumerate(lst):  #enumerate: 열거하다.
    print(f'lst[{index}]: {v}]')
...
# key 속성 -> 정렬 기준 함수
print("---key속성 -> 정렬 기준 함수---")
fruits.append("kiwi")
print("fruits: ", fruits)

# len 함수를 정렬 기준으로 설정
fruits.sort(key=len)
print("정렬 후 fruits: ", fruits) # 같은 것은 알파벳 순으로 정렬됨
...
# d = 0번 인덱스 요소(10)
# *e = 1,2 인덱스 요소 [20, 30] -> d를 제외한 나머지를 list형태로 반환
d, *e = numbers
print(d, e)
